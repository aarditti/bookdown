...
```{r predictor, eval = TRUE, echo = FALSE}
slope<-fires %>% 
  group_by(Slope) %>% 
  tally()
...
lw_data %>% 
  ggplot(mapping = aes(x=as.numeric(Area_m2), fill= "coral"))+
           geom_density()+
    labs(x = "Size of wood pieces (m2)-log10", y="Density")+
   scale_x_log10()+
   theme_dark()+
...
```
This graph shows the majority of wood features are between 1-2 m2. These most likely account for individual pieces.
